...
class RawSqlResultsToEntityTransformer {
    ...
    group(rawResults, alias) {
        ...
        if (alias.metadata.tableType === "view") {
            keys.push(...alias.metadata.columns.map((column) => DriverUtils_1.DriverUtils.buildAlias(this.driver, alias.name, column.databaseName)));
        }
        else {
            keys.push(...alias.metadata.primaryColumns.map((column) => DriverUtils_1.DriverUtils.buildAlias(this.driver, alias.name, column.databaseName)));
        }
    ...
    transformRawResultsGroup(rawResults, alias) {
        ...
        if (metadata.discriminatorColumn) {
            const discriminatorValues = rawResults.map((result) => result[DriverUtils_1.DriverUtils.buildAlias(this.driver, alias.name, alias.metadata.discriminatorColumn.databaseName)]);
    ...
    transformColumns(rawResults, alias, entity, metadata) {
        ...
        metadata.columns.forEach((column) => {
            ...
            if (metadata.childEntityMetadatas.length > 0 &&
                ...
                return;
            const value = rawResults[0][DriverUtils_1.DriverUtils.buildAlias(this.driver, alias.name, column.databaseName)];
    ...
    transformRelationCounts(rawSqlResults, alias, entity) {
        ...
        this.rawRelationCountResults
            ...
            const referenceColumnValue = rawSqlResults[0][DriverUtils_1.DriverUtils.buildAlias(this.driver, alias.name, referenceColumnName)]; // we use zero index since its grouped data // todo: selection with alias for entity columns wont work
            ...
            rawSqlResults.forEach((rawSqlResult) => {
                if (relation.isManyToOne || relation.isOneToOneOwner) {
                    valueMap[column.databaseName] =
                        this.driver.prepareHydratedValue(rawSqlResult[DriverUtils_1.DriverUtils.buildAlias(this.driver, parentAlias, column.databaseName)], column);
                }
                else {
                    valueMap[column.databaseName] =
                        this.driver.prepareHydratedValue(rawSqlResult[DriverUtils_1.DriverUtils.buildAlias(this.driver, parentAlias, column.referencedColumn.databaseName)], column);
                }
            });
            return valueMap;
        }, {});
